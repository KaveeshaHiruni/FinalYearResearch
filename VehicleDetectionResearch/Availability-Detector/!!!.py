...
while True:
    ...
    # Traffic Control Logics
    # The traffic control logic 01
    if elapsed_time <= 40:
        if bare_roads_detected and availability:
            # Turn the traffic light to red
            traffic_light_state = 0  # Red

    # The traffic control logic 02
    if elapsed_time <= 40:
        if availability:
            # Turn the traffic light to red
            traffic_light_state = 0  # Red
    ...
    if elapsed_time <= 20:
        traffic_light_state = 2  # Green
        # Check if there are any vehicles with speed above 30 m/s
        if any(speed > 20.0 for speed in vehicleSpeeds.values()):
            # If at least one vehicle is speeding, set the flag to True
            speeding_vehicle_detected = True
            if speeding_vehicle_detected:
                traffic_light_state = 2  # Green

    if elapsed_time > 20 and not speeding_vehicle_detected:
        traffic_light_state = 0  # Red
