...
cap = cv2.VideoCapture("../Videos/videoScenario.mp4")  # No bare road and available instance within first 30s
# To show the 2nd logic
...
red_start_time = time.time()  # Initialize red_start_time
Yellow_start_time = time.time()
yellow_start_time = time.time()  # Initialize yellow_start_time to None
Yellow_start_time_three = time.time()
...
traffic_light_condition_two = False
...
while True:
    ...
    if traffic_light_state == 2 and elapsed_time <= 50:
        if bare_roads_detected and availability:
    ...
    # Traffic Control Logics 02
    if 50 < elapsed_time <= 200:
        if traffic_light_state == 2:
            if availability:
                # Turn the traffic light to red if it wasn't already red
                if traffic_light_state != 0:
                    traffic_light_state = 1  # Yellow
                    yellow_start_time = time.time()  # Record the start time when the light turns red
        elif traffic_light_state == 1:  # Check if it's time to turn the light yellow
            elapsed_yellow_time = time.time() - yellow_start_time
            if elapsed_yellow_time >= 4:
                traffic_light_state = 0  # Green

    # The traffic control logic 03
    if not traffic_light_condition and not traffic_light_condition_two and 50 < elapsed_time <= 60 \
            and any(speed > 10.0 for speed in vehicleSpeeds.values()):
    ...
    if not traffic_light_condition and elapsed_time > 60:
        if traffic_light_state == 2:
            if not speeding_vehicle_detected:
                traffic_light_state = 0  # Yellow
                Yellow_start_time_three = time.time()
        elif traffic_light_state == 1:
            elapsed_yellow_time_three = time.time() - Yellow_start_time_three
            if elapsed_yellow_time_three >= 4:
                traffic_light_state = 0  # Red
