...
def capture_screenshot(img1, a1, b1, a2, b2, id, speed):
    ...
    # Display a message_speed when a screenshot is captured
    print(f"Screenshot captured for vehicle {id} with speed {speed:.2f} m/s")

    # Display the message_speed at the bottom of the image
    message_speed = f"Vehicle {id} captured with speed {speed:.2f} m/s"
    message_position = (10, img1.shape[0] - 30)  # Position at the bottom of the image
    cv2.putText(img1, message_speed, message_position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
url = 'https://edusense-59f5b618df4b.herokuapp.com/light/getPedTime'
data = -99
while data == -99 or data == '-99':
...
while True:
    ...
    draw_traffic_light_with_rectangle(img_road, temp_state)

    # Traffic control logic 01
    if round1 == 0 and cumulative_condition and cumulative_availability:
        start_time = time.time()
    ...
    cv2.putText(img_road, time_str, timer_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
    cv2.putText(img_road2, time_str, timer_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
    if redRound1 == 1:
    ...
    if temp_state == 0 and elapsed_time == int(data):  # need to change
        temp_state = 1
        yellowTime = elapsed_time + 10
        if detec1:
            print(temp_state)
            detec1 = False
            ...
            response = requests.get(url)
            message = response.json()
    if temp_state == 1 and elapsed_time == yellowTime:
        temp_state = 2
        if detec2:
            print(temp_state)
            detec2 = False
