...
# cap_road = cv2.VideoCapture("../Videos/lowresFullRoad.mp4")  # for road video
# cap_road2 = cv2.VideoCapture("../Videos/lowresFullRoad.mp4")  # for road2 video
# Second Logic
cap_road = cv2.VideoCapture("../Videos/scenarioCombined2.mp4")  # for road video
cap_road2 = cv2.VideoCapture("../Videos/scenarioCombined2.mp4")  # for road2 video
...
while True:
    ...
    for result in resultsTracker_road:
        ...
        if id in vehicleCrossingTimes and (cy > limits1[1] or cx > limits1[2]):
            ...
            # Check if the speed exceeds 10 m/s and capture a screenshot
            if speed > 10.0:
    ...
    if traffic_light_state == 2 and elapsed_time <= 60:
    ...
    if traffic_light_state == 2 and 60 <= elapsed_time <= 120:
        if cumulative_availability:
            # Turn the traffic light to red if it wasn't already red
            if traffic_light_state != 0:
                traffic_light_state = 0  # Red
                red_start_time = time.time()  # Record the start time when the light turns red
    ...
    # if 150 < elapsed_time <= 200 and high_speed_detected_one and high_speed_detected_two:
    ...
    #     cv2.putText(combined_img, high_speed_text, high_speed_text_position,
