...
cap = cv2.VideoCapture("../Videos/traffic.mp4")  # for videos
...
limits = [100, 50, 700, 50]
limits1 = [100, 360, 700, 360]
totalCount = []
...
while True:
    ...
    for result in resultsTracker:
        ...
        if id in vehicleCrossingTimes and cy > limits1[1]:
            # Calculate time elapsed
            timeElapsed = (cv2.getTickCount() - vehicleCrossingTimes[id]) / cv2.getTickFrequency()
            speed = 10 / timeElapsed  # Calculate speed (distance / time)
            ...
            # cv2.putText(img, speed_text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
