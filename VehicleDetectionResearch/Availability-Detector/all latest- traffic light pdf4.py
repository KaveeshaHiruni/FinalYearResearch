...
# Initialize the start time for the first 20 seconds
start_time_first_20s = time.time()

# Initialize a flag to indicate if vehicles were detected during the first 20 seconds
...
# Initialize a flag to indicate if the traffic light has changed from red to yellow
traffic_light_changed = False
...
while True:
    ...
    if len(resultsTracker) > 0:
        last_vehicle_detection_time = time.time()  # Update the last detection time
        vehicles_detected = True  # Set the flag to True if vehicles are detected

        # If the traffic light is currently red, change it to yellow
        if traffic_light_state == 0 and not traffic_light_changed:
            traffic_light_state = 1  # Change the traffic light to yellow
            traffic_light_changed = True  # Set the flag to indicate the change
            yellow_start_time = time.time()  # Start counting the time in yellow state

        # If the traffic light has been yellow for 4 seconds, change it to green
        if traffic_light_state == 1 and yellow_start_time is not None:
            elapsed_yellow_time = time.time() - yellow_start_time
            if elapsed_yellow_time >= 5:
                traffic_light_state = 2  # Change the traffic light to green

    else:
        # If there were no vehicles detected during the first 20 seconds, change the traffic light to red instantly
        if not vehicles_detected and time.time() - start_time_first_20s < 20:
            traffic_light_state = 0  # Change the traffic light to red
            traffic_light_changed = False  # Reset the traffic light change flag
            yellow_start_time = None  # Reset the yellow start time

    # Once the first 20 seconds have passed, reset the flag and start time
    if time.time() - start_time_first_20s >= 20:
        vehicles_detected = False
        start_time_first_20s = time.time()
