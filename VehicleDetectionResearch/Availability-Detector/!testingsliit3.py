...
# cap_road = cv2.VideoCapture("../Videos/2Overall.mp4")  # No bare road and available instance within first 30s
# cap_road2 = cv2.VideoCapture("../Videos/2Overall.mp4")  # No bare road and available instance within first 30s
# previous
# cap_road = cv2.VideoCapture("../Videos/lowresFullRoad.mp4")  # No bare road and available instance within first 30s
# cap_road2 = cv2.VideoCapture("../Videos/resLow.mp4")  # No bare road and available instance within first 30s
cap_road = cv2.VideoCapture("../Videos/scenario3.mp4")  # No bare road and available instance within first 30s
cap_road2 = cv2.VideoCapture("../Videos/resLow.mp4")  # No bare road and available instance within first 30s
...
speeding_vehicle_detected = False
high_speed_detected_one = False
high_speed_detected_two = False
...
url = 'https://edusense-59f5b618df4b.herokuapp.com/light/set/g'
response = requests.get(url)
if response.json() == "G activated":
...
# Define a green timer variable and initialize it to zero
green_timer = 0
green_time_remaining = 0  # Keep track of the remaining green time
green_timer_two = 0
green_timer_one = 0
...
# Create a lock to synchronize access to the timer and display
timer_lock = threading.Lock()


# Function to update the timer and change traffic_light_state
def update_timer_and_state():
    global green_timer_one, traffic_light_state, img_road, detec0, url, response

    while green_timer_one > 0:
        # Acquire the lock to ensure synchronized access
        with timer_lock:
            # Format the time_to_intersection for display (e.g., in seconds)
            time_to_intersection_text = f"Time: {green_timer_one:.2f} s"
            time_to_intersection_text_position = (10, 180)  # Adjust the position as needed
            cv2.putText(img_road, time_to_intersection_text, time_to_intersection_text_position,
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
            # Decrement the green_timer_one by 1 second
            green_timer_one -= 1  # You can adjust the time step as needed
        time.sleep(1)  # Delay for 1 second
    # url = 'https://edusense-59f5b618df4b.herokuapp.com/light/set/r'
    # response = requests.get(url)
    # if response.json() == "R activated":
    #     traffic_light_state = 0
    # if detec0:
    url1 = 'https://edusense-59f5b618df4b.herokuapp.com/light/getAmb'
    ...
    url = 'https://edusense-59f5b618df4b.herokuapp.com/light/set/r'
    response = requests.get(url)
    if response.json() == "R activated" and traffic_light_state != 0:
        traffic_light_state = 0
...
while True:
    ...
    for result in resultsTracker_road:
        ...
        if id in vehicleCrossingTimes and (cy > limits1[1] or cx > limits1[2]):
            ...
            # Check if the speed exceeds 11.0 m/s and capture a screenshot
            if speed > 7.0:
                capture_screenshot(img_road, x1, y1, x2, y2, id, speed)
                if 0 < elapsed_time <= 120 and traffic_light_state == 2:
                    # traffic_light_state = 1
                    if detec1:
                        url = 'https://edusense-59f5b618df4b.herokuapp.com/light/set/y'
                        response = requests.get(url)
                        if response.json() == "Y activated":
                            traffic_light_state = 1
                            print(traffic_light_state)
                            detec1 = False
                        high_speed_detected_one = True
                        high_speed_value_one = speed
                        distance_to_intersection = 100  # Set a static distance of 20 for speeding vehicles

                        # Calculate the time it will take for the speeding vehicle to reach the intersection
                        time_to_intersection_one = distance_to_intersection / high_speed_value_one
                        green_timer_one = math.ceil(time_to_intersection_one)  # Set the green timer to this time

                        # Create a separate thread to update the timer and change the state
                        timer_thread = threading.Thread(target=update_timer_and_state)
                        timer_thread.start()
                        break
            ...
            if speed > 7.0:
                ...
                if 0 < elapsed_time <= 120 and traffic_light_state == 2:
                    if detec1:
                        ...
                        if response.json() == "Y activated":
                            traffic_light_state = 1
                        high_speed_detected_two = True
                        high_speed_value_two = speed
                        distance_to_intersection = 100  # Set a static distance of 20 for speeding vehicles

                        # Calculate the time it will take for the speeding vehicle to reach the intersection
                        time_to_intersection_one = distance_to_intersection / high_speed_value_two
                        green_timer_one = math.ceil(time_to_intersection_one)  # Set the green timer to this time
    ...
    if availability_road:
        ...
        cv2.putText(img_road, "Unavailable", (img_road.shape[1] - 240, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2,
                    cv2.LINE_AA)
    ...
    if 0 < elapsed_time <= 30 and high_speed_detected_one or high_speed_detected_two:
        # If at least one vehicle is speeding, set the flag to True
        speeding_vehicle_detected = True
        high_speed_text = "High Speed!!"
        high_speed_text_position = (10, 150)  # Adjust the position as needed
        cv2.putText(combined_img, high_speed_text, high_speed_text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255),
                    2, cv2.LINE_AA)
